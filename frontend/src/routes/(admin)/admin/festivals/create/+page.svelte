<script lang="ts">
	...
	import type { FestivalStatus } from "$lib/types/festivals";
	...
	const formAction = $derived(
		isEditing && data.festivalId
			? `?/update&edit=${data.festivalId}`
			: "?/create",
	);
...
<section class="create-shell animate-fade-in">
	<div class="page-intro">
		<div>
			<p class="eyebrow">Festivals Management</p>
			<h1>{isEditing ? "Edit Festival" : "Create Festival"}</h1>
			<p class="lede">
				{isEditing
					? "Update this festival's details below."
					: "Fill in the festival details to publish it on the calendar."}
			</p>
		</div>
	</div>
		...
		<form method="POST" action={formAction} class="event-form">
			<section class="panel">
				...
				<label>
					...
					<RichTextEditor
						bind:value={values.longDescription}
						placeholder="Describe the festival details..."
					/>
				</label>
			</section>
			...
			<section class="panel">
				...
				<div class="holiday-toggle-field">
					<span class="field-label"><i class="fi fi-rr-calendar-star"></i> Public Holiday</span>
					<label class="toggle-switch">
						<input
							name="isHoliday"
							type="checkbox"
							bind:checked={values.isHoliday}
						/>
						<span class="toggle-track">
							<span class="toggle-thumb"></span>
						</span>
						<span class="toggle-label">
							{values.isHoliday ? "Yes — shown in red on calendar" : "No — normal calendar display"}
						</span>
					</label>
				</div>
			</section>
			...
			<section class="panel">
				...
				<div class="map-field-wrapper">
					...
					<span>Festival Location Pin</span>
					<LocationPickerMap
			...
			<section class="panel">
				...
				<div class="image-upload-wrapper">
					...
					<div class="uploader-header">
						<span>Upload Festival Images *</span>
					...
					<div class="uploaded-image-preview-grid">
						{#each uploadedImages as img, idx}
							<div class="uploaded-image-preview-card">
								<img src={img} alt="Festival preview" />
								<button
									type="button"
			...
			<div class="actions">
				<button class="submit-btn" type="submit"
					>{isEditing ? "Update Festival" : "Create Festival"}</button
				>
			</div>
		</form>
